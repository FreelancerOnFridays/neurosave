...
function isOverdue(iso: string | null): boolean {
  ...
  const d = new Date(iso);
  if (isDateOnly(iso)) {
    // Date-only deadline: overdue only when the calendar date has passed (local timezone)
...
function PersonalTaskCard({ task, onDone, onDelete, onSetReminder, onDeleteReminder }: PersonalTaskCardProps) {
  ...
  const accentStrip = overdue ? "bg-tg-destructive" : "bg-tg-accent";
        ...
        <div className="bg-tg-secondary rounded-2xl overflow-hidden flex">
          ...
          <div className="flex-1 p-4 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <p className="text-sm font-semibold text-tg-text flex-1 leading-snug">{task.description}</p>
              <button
                onClick={onDone}
            ...
            {(task.deadline || reminderFuture) && (
              <div className="flex flex-col gap-0.5 mt-1.5">
                {task.deadline && (
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs leading-none">📅</span>
                    <span className={`text-xs font-medium ${overdue ? "text-tg-destructive" : "text-tg-hint"}`}>
                      Дедлайн:
                    </span>
                    <span className={`text-xs ${overdue ? "text-tg-destructive" : "text-tg-hint"}`}>
                ...
                {reminderFuture && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs text-tg-accent">{formatReminderTime(task.reminder_time!)}</span>
                  </div>
                )}
              </div>
            )}
  ...
  return (
    <SwipeAction actions={actions}>
      <div className="bg-tg-secondary rounded-2xl overflow-hidden flex">
        <div className={`w-1 shrink-0 ${accentStrip}`} />
        <div className="flex-1 p-4 min-w-0">
          <p className="text-sm font-semibold text-tg-text leading-snug">{task.description}</p>
          <div className="flex flex-col gap-0.5 mt-1.5">
...
export default function TodayPage() {
  ...
  const summary: TodaySummary = {
    ...
    overdue_count: tasks.filter(
      (t) => t.status === "open" && isOverdue(t.deadline)
    ).length,
